...
LSCV <- function(xx, b, j, method) {
  if (design_option == 1) { # Fixed design
    integrand <- function(s) {
      # Return 0 if s is outside the simplex (sum(s) >= 1)
      if (sum(s) >= 1) {
        return(0)
      } else {
  ...
  } else if (design_option == 2) { # Random design
    integrand <- function(s) {
      # Return 0 if s is outside the simplex (sum(s) >= 1)
      if (sum(s) >= 1) {
        return(0)
      } else {
      return((hat_m(xx, b, s, j, method) - m(j, s))^2 * design_density(s))
      }
    }
  } else {
...
LSCV_MC <- function(xx, b, j, method, unif_sample = NULL) {
  ...
  if (design_option == 1) { # Fixed design
    ...
    for (i in 1:MCsim) {
      integ_squared[i] <- (hat_m(xx, b, unif_sample[i, 1:d], j, method) - m(j, unif_sample[i, 1:d])) ^ 2 * design_density(unif_sample[i, 1:d])
    }
  } else {
    stop("Invalid value for design_option. Must be 1 (fixed design) or 2 (random design).")
  }
