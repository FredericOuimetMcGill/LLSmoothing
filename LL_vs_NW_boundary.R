...
ISE_MC <- function(xx, j, method) {
  ...
  while (nrow(unif_sample) < MCsim) {
    # Generate a batch of Dirichlet samples
    samples <- LaplacesDemon::rdirichlet(MCsim, rep(1, d + 1))
    
    # Filter samples where at least one component is smaller than the threshold
    filtered_samples <- samples[apply(samples, 1, function(x) any(x < threshold)), , drop = FALSE]
    
    # Append filtered samples to the final result
    unif_sample <- rbind(unif_sample, filtered_samples)
  }
